...
// Code this method receives an error, then compares its interface type with the CustomError interface.
...
// Retryable this method receives an error, then compares its interface type with the CustomError interface.
// If the interfaces types matches, returns if the error is Retryable.
